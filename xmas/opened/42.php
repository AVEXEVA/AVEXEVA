...
array_shift($csv);
?>
<div class='card' rel='42'>
  <div class='card-header'>$13.80 Mask</div>
  <img class='card-image-top' src='media/mask2.jpg' width='100%' />
  <div class='card-body'>Outdoor Mask Protective 5-Layer Activated Carbon Filter, Adult Men and Women Headscarves</div>
  <div class='card-footer'>
    <div class='row'>
      <div class='col'><button onClick='' disabled>Claimed by <?php
        if(count($csv) > 0){foreach($csv as $row){
          if($row['Gift'] == 42){echo $row['Person'];}
        }}
      ?></div>
    </div>
    <?php
    ...
    if(count($csv) > 0 && $check){foreach($csv as $row){
      if($row['Gift'] == 42 && $row['Person'] == $_SESSION['Person']){?>
        <div class='row'>
