...
array_shift($csv);
?>
<div class='card' rel='13'>
  <div class='card-header'>$2.50 Cookie Crisp</div>
  <img class='card-image-top' src='media/crisp.jpg' width='100%' />
  <div class='card-body'>Psssssttt... I got that cookie crisp. You in?</div>
  <div class='card-footer'>
    <div class='row'>
      <div class='col'><button onClick='' disabled>Claimed by <?php
        ...
        if(count($csv) > 0){foreach($csv as $row){
          if($row['Gift'] == 13){echo $row['Person'];}
        }}
      ?></div>
    </div>
    <?php 
    ...
    if(count($csv) > 0 && $check){foreach($csv as $row){
      if($row['Gift'] == 13 && $row['Person'] == $_SESSION['Person']){?>
        <div class='row'>
