...
<div class='card' rel='20'>
  <div class='card-header'>$43.88 Catan The Board Game, Multicolor</div>
  <img class='card-image-top' src='media/catan.jpg' width='100%' />
  <div class='card-body'>The incredibly popular, multi award winning civilization building board game of harvesting and trading resources</div>
  <div class='card-footer'>
    <div class='row'>
      <div class='col'><button onClick='' disabled>Claimed by <?php
        if(count($csv) > 0){foreach($csv as $row){
          if($row['Gift'] == 20){echo $row['Person'];}
        }}
      ?></div>
    </div>
    <?php 
    ...
    if(count($csv) > 0 && $check){foreach($csv as $row){
      if($row['Gift'] == 20 && $row['Person'] == $_SESSION['Person']){?>
        <div class='row'>
