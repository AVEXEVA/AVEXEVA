...
array_shift($csv);
?>
<div class='card' rel='29'>
  <div class='card-header'>$57.99 Art Supplies</div>
  <img class='card-image-top' src='media/art.jpg' width='100%' />
  <div class='card-body'>Art Supplies, with Soft & Oil Pastels, Acrylic Paints,Watercolor Paints, Water Color Set, Sketching Kit, Charcoal & Colored Pencils, Watercolor Cakes, and Tools(Wooden), Art Set in Exquisite Wood Box.</div>
  <div class='card-footer'>
    <div class='row'>
      <div class='col'><button onClick='' disabled>Claimed by <?php
        if(count($csv) > 0){foreach($csv as $row){
          if($row['Gift'] == 29){echo $row['Person'];}
        }}
      ?></div>
    </div>
    <?php 
    ...
    if(count($csv) > 0 && $check){foreach($csv as $row){
      if($row['Gift'] == 29 && $row['Person'] == $_SESSION['Person']){?>
        <div class='row'>
