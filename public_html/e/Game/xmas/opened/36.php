...
array_shift($csv);
?>
<div class='card' rel='36'>
  <div class='card-header'>$49.99 Camera Glasses</div>
  <img class='card-image-top' src='media/glasses.jpg' width='100%' />
  <div class='card-body'>Camera Glasses 1080P,HD Video Glasses Max 32GB Memory Card - Eye Glasses with Camera - Wearable Camera</div>
  <div class='card-footer'>
    <div class='row'>
      <div class='col'><button onClick='' disabled>Claimed by <?php
        if(count($csv) > 0){foreach($csv as $row){
          if($row['Gift'] == 36){echo $row['Person'];}
        }}
      ?></div>
    </div>
    <?php
    ...
    if(count($csv) > 0 && $check){foreach($csv as $row){
      if($row['Gift'] == 36 && $row['Person'] == $_SESSION['Person']){?>
        <div class='row'>
