...
array_shift($csv);
?>
<div class='card' rel='41'>
  <div class='card-header'>$14.99 Martini Olive Brine</div>
  <img class='card-image-top' src='media/brine.jpg' width='100%' />
  <div class='card-body'>Calivirgin Dirty Martini Mix - Uses Certified Organic Olive Brine - All-Natural Dirty Martini Juice From Handpicked California Olives - No Preservatives</div>
  <div class='card-footer'>
    <div class='row'>
      <div class='col'><button onClick='' disabled>Claimed by <?php
        if(count($csv) > 0){foreach($csv as $row){
          if($row['Gift'] == 41){echo $row['Person'];}
        }}
      ?></div>
    </div>
    <?php
    ...
    if(count($csv) > 0 && $check){foreach($csv as $row){
      if($row['Gift'] == 41 && $row['Person'] == $_SESSION['Person']){?>
        <div class='row'>
