...
array_shift($csv);
?>
<div class='card' rel='39'>
  <div class='card-header'>$3.00 I'm Real Olive Radiance Mask Sheet</div>
  <img class='card-image-top' src='media/backpack.jpg' width='100%' />
  <div class='card-body'>TONYMOLY I'm Real Olive Radiance Mask Sheet</div>
  <div class='card-footer'>
    <div class='row'>
      <div class='col'><button onClick='' disabled>Claimed by <?php
        if(count($csv) > 0){foreach($csv as $row){
          if($row['Gift'] == 39){echo $row['Person'];}
        }}
      ?></div>
    </div>
    <?php
    ...
    if(count($csv) > 0 && $check){foreach($csv as $row){
      if($row['Gift'] == 39 && $row['Person'] == $_SESSION['Person']){?>
        <div class='row'>
