...
<div class='card' rel='16'>
  <div class='card-header'>$69.99 : Projector</div>
  <img class='card-image-top' src='media/projector.jpg' width='100%' />
  <div class='card-body'> Crosstour Mini Portable Movie Projector Support 1080P Home Theater, Video Projector with 55,000 Hrs LED Lamp Life, Compatible with HDMI, RCA, USB, TF, TV Stick, PS4, iPhone</div>
  <div class='card-footer'>
    <div class='row'>
      <div class='col'><button onClick='' disabled>Claimed by <?php
        if(count($csv) > 0){foreach($csv as $row){
          if($row['Gift'] == 16){echo $row['Person'];}
        }}
      ?></div>
    </div>
    <?php 
    ...
    if(count($csv) > 0 && $check){foreach($csv as $row){
      if($row['Gift'] == 16 && $row['Person'] == $_SESSION['Person']){?>
        <div class='row'>
