...
<div class='card' rel='14'>
  <div class='card-header'>$34.95 The Original Buddha Board</div>
  <img class='card-image-top' src='media/buddha.jpg' width='100%' />
  <div class='card-body'>The Original BUDDHA BOARD:Relaxing Water Painting with Bamboo Brush & Stand, Mindfulness & Meditation Practice, Environmentally Friendly.Zen Concept Live in The Moment. Master The Art of letting go.</div>
  <div class='card-footer'>
    <div class='row'>
      <div class='col'><button onClick='' disabled>Claimed by <?php
        if(count($csv) > 0){foreach($csv as $row){
          if($row['Gift'] == 14){echo $row['Person'];}
        }}
      ?></div>
    </div>
    <?php 
    ...
    if(count($csv) > 0 && $check){foreach($csv as $row){
      if($row['Gift'] == 14 && $row['Person'] == $_SESSION['Person']){?>
        <div class='row'>
