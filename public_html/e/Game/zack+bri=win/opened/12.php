...
array_shift($csv);
?>
<div class='card' rel='12'>
  <div class='card-header'>$60 Shiatsu Massager</div>
  <img class='card-image-top' src='media/massage.jpg' width='100%' />
  <div class='card-body'>Zyllion Shiatsu Back and Neck Massager - Kneading Massage Pillow with Heat for Shoulders, Lower Back, Calf - Use at Home and Car, Black, (ZMA-13-BK)</div>
  <div class='card-footer'>
    <div class='row'>
      <div class='col'><button onClick='' disabled>Claimed by <?php
        ...
        if(count($csv) > 0){foreach($csv as $row){
          if($row['Gift'] == 12){echo $row['Person'];}
        }}
      ?></div>
    </div>
    <?php 
    ...
    if(count($csv) > 0 && $check){foreach($csv as $row){
      if($row['Gift'] == 12 && $row['Person'] == $_SESSION['Person']){?>
        <div class='row'>
