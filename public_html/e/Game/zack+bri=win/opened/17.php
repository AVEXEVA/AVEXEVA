...
array_shift($csv);
?>
<div class='card' rel='17'>
  <div class='card-header'>$12.95 : 3D Skull Flexible Silicone Ice Cube Mold</div>
  <img class='card-image-top' src='media/skull.jpg' width='100%' />
  <div class='card-body'>Makes Four Giant Skulls, Round Ice Cube Maker (Black)</div>
  <div class='card-footer'>
    <div class='row'>
      <div class='col'><button onClick='' disabled>Claimed by <?php
        if(count($csv) > 0){foreach($csv as $row){
          if($row['Gift'] == 17){echo $row['Person'];}
        }}
      ?></div>
    </div>
    <?php 
    ...
    if(count($csv) > 0 && $check){foreach($csv as $row){
      if($row['Gift'] == 17 && $row['Person'] == $_SESSION['Person']){?>
        <div class='row'>
