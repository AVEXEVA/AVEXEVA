...
<div class='card' rel='1'>
  <div class='card-header'>$100 Bitcoin</div>
  <img class='card-image-top' src='media/cryptocurrencies.jpg' width='100%' />
  <div class='card-body'>It is a decentralized digital currency without a central bank or single administrator that can be sent from user to user on the peer-to-peer bitcoin network without the need for intermediaries. </div>
  <div class='card-footer'>
    ...
    if(count($csv) > 0 && $check){foreach($csv as $row){
      if($row['Gift'] == 1 && $row['Person'] == $_SESSION['Person']){?>
        <div class='row'>
