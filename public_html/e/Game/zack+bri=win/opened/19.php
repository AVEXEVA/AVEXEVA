...
array_shift($csv);
?>
<div class='card' rel='14'>
  <div class='card-header'>$12.99 Akwox Wooden Cable Organizer and Cord Management System</div>
  <img class='card-image-top' src='media/cords.jpg' width='100%' />
  <div class='card-body'>Good for all surfaces, Quick peel and stick: Ideal tidy up earphone Wire, Electric Wire, usb Cable or other accessory items.</div>
  <div class='card-footer'>
    <div class='row'>
      <div class='col'><button onClick='' disabled>Claimed by <?php
        if(count($csv) > 0){foreach($csv as $row){
          if($row['Gift'] == 19){echo $row['Person'];}
        }}
      ?></div>
    </div>
    <?php 
    ...
    if(count($csv) > 0 && $check){foreach($csv as $row){
      if($row['Gift'] == 19 && $row['Person'] == $_SESSION['Person']){?>
        <div class='row'>
