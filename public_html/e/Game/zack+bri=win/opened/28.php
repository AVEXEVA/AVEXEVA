...
array_shift($csv);
?>
<div class='card' rel='28'>
  <div class='card-header'>$25 Home Depot Gift Card</div>
  <img class='card-image-top' src='media/homedepot.png' width='100%' />
  <div class='card-body'>ZOMG! TIME TO BUY SOME LUMBER!</div>
  <div class='card-footer'>
    <div class='row'>
      <div class='col'><button onClick='' disabled>Claimed by <?php
        if(count($csv) > 0){foreach($csv as $row){
          if($row['Gift'] == 28){echo $row['Person'];}
        }}
      ?></div>
    </div>
    <?php 
    ...
    if(count($csv) > 0 && $check){foreach($csv as $row){
      if($row['Gift'] == 28 && $row['Person'] == $_SESSION['Person']){?>
        <div class='row'>
