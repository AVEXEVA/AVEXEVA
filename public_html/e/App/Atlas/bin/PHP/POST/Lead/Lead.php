		...
		if(isset($_POST['action']) && $_POST['action'] == 'edit'){
			if(isset($_POST['data']) && count($_POST['data']) > 0){
				...
				foreach($_POST['data'] as $ID=>$Lead){
					$r = sqlsrv_query($NEI,"SELECT OwnerWithRol.ID AS ID FROM nei.dbo.OwnerWithRol WHERE OwnerWithRol.Name = ?;",array($Lead['Customer']));
			...
			if(isset($_POST['data']) && count($_POST['data']) > 0){
				...
				foreach($_POST['data'] as $ID=>$Lead){
					...
					$resource = sqlsrv_query($NEI,"SELECT Max(Lead.ID) AS ID FROM nei.dbo.Lead;");
					$Lead_Primary_Key = sqlsrv_fetch_array($resource)['ID'];
					$Lead_Primary_Key++;
					sqlsrv_query($NEI,"INSERT INTO nei.dbo.Rol(ID, fDesc, RolType, Rol, Fax, Remarks, Type, fLong, Latt, GeoLock, Since, Last, EN, Cellular, Country, Contact, EMail,Website) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)",array($Lead_Primary_Key, $Lead['Tag'], "NY", $Lead['Contact_Phone'], $Lead['Contact_Fax'],"",4,0,0,0,'2018-01-22 00:00:00.000','2018-01-22 00:00:00.000',1,"(","United States",$LocLeadation['Contact_Name'],$Lead['Contact_Email'],$Lead['Contact_Website']));
