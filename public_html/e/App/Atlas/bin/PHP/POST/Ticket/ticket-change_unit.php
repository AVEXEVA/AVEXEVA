...
if(isset($_SESSION['User'],$_SESSION['Hash'])){
  ...
  if(!$Privileged || count($_POST) == 0 || !isset($_POST['ID']) || !is_numeric($_POST['ID'])){?><html><head><script>document.location.href='../login.php';</script></head></html><?php }
    ...
    if($r && !is_array($ticket2)){
      ...
      if(is_array($row)){
        sqlsrv_query($NEI,"UPDATE nei.dbo.TicketO SET TicketO.LElev = ? WHERE TicketO.ID = ?;",array($_POST['Unit'],$_POST['ID']));
      }
    } elseif($r && is_array($ticket2)){
      $row = sqlsrv_fetch_array($r);
      if(is_array($row)){
        sqlsrv_query($NEI,"UPDATE nei.dbo.TicketO SET TicketO.Elev = ? WHERE TicketO.ID = ?;",array($_POST['Unit'],$_POST['ID']));
        sqlsrv_query($NEI,"UPDATE nei.dbo.TicketDPDA SET TicketDPDA.Elev = ? WHERE TicketDPDA.ID = ?",array($_POST['Unit'],$_POST['ID']));
      }
    }
  }
}?>
