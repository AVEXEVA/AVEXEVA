...
if(isset($_SESSION['User'],$_SESSION['Hash'])){
    ...
    if(!isset($array['ID'],$_POST['ID'])  || !$Privileged && is_numeric($_POST['ID'])){?><html><head></head></html><?php }
      ...
      if(isset($_POST['Internal_Comments']) && strlen($_POST['Internal_Comments']) > 0){
        $r = sqlsrv_query($NEI,"SELECT * FROM TicketDPDA WHERE ID = ?",array($_POST['ID']));
        if($r && is_array(sqlsrv_fetch_array($r))){
          $sQuery = "UPDATE nei.dbo.TicketDPDA SET TicketDPDA.Comments = ? WHERE TicketDPDA.ID = ?;";
          sqlsrv_query($NEI, $sQuery,array($_POST['Internal_Comments'],$_POST['ID']));
          ...
          $sQuery = "UPDATE nei.dbo.TicketD SET TicketD.Comments = ? WHERE TicketD.ID = ?;";
          sqlsrv_query($NEI, $sQuery,array($_POST['Internal_Comments'],$_POST['ID']));
        }
