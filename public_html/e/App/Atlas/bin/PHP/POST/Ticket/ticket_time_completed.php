...
if(isset($_SESSION['User'],$_SESSION['Hash'])){
    ...
    if(!isset($array['ID'], $_POST['ID'])  || !$Privileged || !is_numeric($_POST['ID'])){?><html><head></head></html><?php }
      ...
      sqlsrv_query($Portal,"INSERT INTO Activity([User], [Date], [Page]) VALUES(?,?,?);",array($_SESSION['User'],date("Y-m-d H:i:s"), "ticket_time_completed.php?ID={$_POST['ID']}"));
      ...
      echo $post_time;
      sqlsrv_query($NEI,"UPDATE nei.dbo.TicketO SET TicketO.EDate = ?, TicketO.TimeComp = ?, TicketO.Assigned = 3, TicketO.Confirmed = 1, TicketO.HandheldFieldsUpdated = 1, TicketO.ResolveSource = 'TFM-A3.60' WHERE TicketO.ID = ?;",array(date("Y-m-d H:i:s"), $post_time,$_POST['ID']));
      //sqlsrv_query($NEI,"")
    }
}?>
