...
if(isset($_SESSION['User'],$_SESSION['Hash'])){
    ...
    if(!isset($array['ID'], $_POST['ID']) || !$Privileged || !is_numeric($_POST['ID'])){?><html><head></head></html><?php }
      ...
      $r = sqlsrv_query($NEI,"SELECT * FROM TicketO LEFT JOIN nei.dbo.Emp ON TicketO.fWork = Emp.fWork WHERE Emp.ID = ? AND TicketO.ID = ?",array($_SESSION['User'],$_POST['ID']));
      if($r && is_array(sqlsrv_fetch_array($r))){
        sqlsrv_query($NEI,"UPDATE nei.dbo.TicketO SET TicketO.TimeRoute = ?, TicketO.TimeSite = ?, TicketO.TimeComp = ?, TicketO.Assigned = 1 WHERE TicketO.ID = ?;",array("1899-12-30 00:00:00.000","1899-12-30 00:00:00.000","1899-12-30 00:00:00.000", $_POST['ID']));
      }
      //sqlsrv_query($NEI,"")
