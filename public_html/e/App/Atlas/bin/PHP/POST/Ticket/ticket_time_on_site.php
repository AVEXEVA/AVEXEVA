    ...
    if(!isset($array['ID'], $_POST['ID'])  || !$Privileged || !is_numeric($_POST['ID'])){?><html><head></head></html><?php }
      ...
      if($r && is_array(sqlsrv_fetch_array($r))){
        ...
        if(!$r || ($r && !is_array(sqlsrv_fetch_array($r)))){
          ...
          if(isset($En_Route)){
            sqlsrv_query($NEI,"UPDATE nei.dbo.TicketO SET TicketO.TimeRoute = ?, TicketO.Assigned = 2, TicketO.EDate = ? WHERE TicketO.ID = ?;",array($En_Route,$En_Route2,$_POST['ID']));
        ...
        sqlsrv_query($NEI,"UPDATE nei.dbo.TicketO SET TicketO.TimeSite = ?, TicketO.Assigned = 3 WHERE TicketO.ID = ?;",array($Time_Site, $_POST['ID']));
        ...
        $r = sqlsrv_query($NEI,"SELECT Loc.Latt AS Latitude, Loc.fLong AS Longitude FROM nei.dbo.Loc LEFT JOIN nei.dbo.TicketO ON Loc.Loc = TicketO.LID WHERE TicketO.ID = ?;",array($_POST['ID']));
        $Location_GPS = Null;
        ...
        if(is_array($Location_GPS) && is_numeric($Location_GPS['Latitude']) && is_numeric($Location_GPS['Longitude']) && $Location_GPS['Latitude'] != 0 && $Location_GPS['Longitude'] != 0){
          ...
          sqlsrv_query($Portal_44,
            " INSERT INTO Portal.dbo.Geofence(Employee_ID, Ticket_ID, Time_Stamp, Distance)
              VALUES(?, ?, ?, ?)
