...
?><div class="panel panel-primary">
  <div class="panel-heading"><h4><div style='float:left;' onClick="document.location.href='home.php';"><?php $Icons->Customer();?> Category Tests</div><div style='clear:both;'></div></h4></div>
  <div class="panel-heading">
    <?php if($_SESSION['User'] != 975){?><button onClick='printSelected();' style='width:250px;height:25px;color:black !important;'>Print & To Sales Selected</button><?php }?>
    <button onClick='printSelected2();' style='width:150px;height:25px;color:black !important;'>Print Selected</button>
    <?php if($_SESSION['User'] != 975){?><button onClick='viewSelected();' style='width:150px;height:25px;color:black !important;'>View Selected</button><?php }?>
    <?php if($_SESSION['User'] != 975){?><button onClick='voidSelected();' style='width:150px;height:25px;color:black !important;'>Void Selected</button><?php }?>
  </div>
  <div class="panel-body">
    <table id='Table_Category_Tests' class='display' cellspacing='0' width='100%' style='font-size:12px;'>
      <thead>
        <th>ID</th>
        <th>Location</th>
        <th>Unit</th>
...
"ajax": "cgi-bin/php/get/Category_Tests2.php",
"columns": [
  {
    data:"ID"
  },{
...
"initComplete":function(){
  $("table#Table_Category_Tests tbody tr[role='row']").on("dblclick", function(){
    if($(this).children(":nth-child(9)").html() > 0){
      openInNewTab('job.php?ID=' + $(this).children(':nth-child(9)').html());
    } else {
      openInNewTab('category_test.php?ID=' + $(this).children(':first-child').html());
    }
  });
},
"scrollY" : "600px",
...
yadcf.init(Table_Category_Tests,[
{
  column_number:3,
