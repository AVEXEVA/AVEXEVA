...
if(isset($_SESSION['User'],$_SESSION['Hash'])){
    ...
    if(!isset($array['ID'])  || !$Privileged || !is_numeric($_GET['ID'])){?><html><head><script>document.location.href="../login.php?Forward=unit<?php echo (!isset($_GET['ID']) || !is_numeric($_GET['ID'])) ? "s.php" : ".php?ID={$_GET['ID']}";?>";</script></head></html><?php }
        ...
        $params = array($_GET['ID'], 'Destination_Dispatch');
        ...
        if($r){
          ...
          if($r){
            ...
            $sQuery = "SELECT Destination_Dispatch.* FROM Device.dbo.Destination_Dispatch WHERE Destination_Dispatch.Item = ?";
            $params = array($Item['ID']);
            $r = sqlsrv_query($database_Device, $sQuery, $params);
            if($r){
              $Destination_Dispatch = sqlsrv_fetch_array($r);
            }
          }
        }
?><!DOCTYPE html>
			<div class="panel panel-primary">
        <div class='panel-heading' onClick="someFunction(this,'unit-destination_dispatch.php?ID=<?php echo $_GET['ID'];?>');"><img src='media/images/icons/destination_dispatch.png' width='auto' height='35px' /> Destination Dispatch</div>
				<div class='panel-body' style='padding-top:10px;'><form id='form_Destination_Dispatch'>
          <?php if(isset($Item)){?><input type='hidden' value='<?php echo $Item['ID'];?>' name='Item' /><?php }?>
          <div class='row'>
            <div class='col-xs-4'>Location:</div>
            <div class='col-xs-8' onclick="someFunction(this,'unit-floors.php?ID=<?php echo $_GET['ID'];?>');"><img src='media/images/icons/floor.png' width='auto' height='25px' /> Floors</div>
          </div>
          <div class='row'>
          ...
          <div class='row'>
            ...
            <div class='col-xs-8'><input type='text' placeholder='Software' value='<?php echo isset($Destination_Dispatch['Software']) ? $Destination_Dispatch['Software'] : '';?>' name='Software' /></div>
          </div>
          <div class='row'>
            <div class='col-xs-4'>Composition:</div>
            <div class='col-xs-8'><input type='text' name='Composition' placeholder='Composition' value='<?php echo isset($Destination_Dispatch['Composition']) ? $Destination_Dispatch['Composition'] : '';?>' /></div>
          </div>
          <div class='row'><div class='col-xs-12'>&nbsp;</div></div>
          <div class='row'><div class='col-xs-12'>&nbsp;</div></div>
          <div class='row'><div class='col-xs-12'><button onClick="save_Destination_Dispatch(this);" type='button' style='width:100%;height:42px;'>Save</button></div></div>
          <div class='row'><div class='col-xs-12'>&nbsp;</div></div>
          <div class='row'><div class='col-xs-12'>&nbsp;</div></div>
        </form></div>
        <script>
          function save_Destination_Dispatch(link){
            $(link).html("Saving <img src='media/images/spinner.gif' height='25px' width='auto' />");
            $(link).attr('disabled','disabled');
            var formElement = document.getElementById('form_Destination_Dispatch');
            var formData = new FormData(formElement);
            formData.append('ID', '<?php echo $_GET['ID'];?>');
            if($("#form_Destination_Dispatch input:invalid").length == 0){
              $.ajax({
                url:"cgi-bin/php/post/unit/destination_dispatch.php",
                data:formData,
                processData: false,
                contentType: false,
                timeout:15000,
        ...
        <div class='panel-body'>
          <div class='row'>
            ...
            <div class='Home-Screen-Option col-lg-1 col-md-2 col-xs-3' onclick="someFunction(this,'unit-center-junction-box.php?ID=<?php echo $_GET['ID'];?>');">
            ...
            <div class='Home-Screen-Option col-lg-1 col-md-2 col-xs-3' onclick="someFunction(this,'unit-car.php?ID=<?php echo $_GET['ID'];?>');">
