...
if(isset($_SESSION['User'],$_SESSION['Hash'])){
    ...
    if(!isset($array['ID'])  || !$Privileged || !is_numeric($_GET['ID'])){?><html><head><script>document.location.href="../login.php?Forward=unit<?php echo (!isset($_GET['ID']) || !is_numeric($_GET['ID'])) ? "s.php" : ".php?ID={$_GET['ID']}";?>";</script></head></html><?php }
        ...
        $params = array($_GET['ID'], 'Generator');
...
?><!DOCTYPE html>
        ...
        <div class='panel-heading' onclick="someFunction(this,'unit-generator.php?ID=<?php echo $_GET['ID'];?>');"><img src='media/images/icons/generator.png' width='auto' height='35px' /> Generator</div>
				<div class='panel-body' style='padding-top:10px;'><form id='form_Generator'>
          ...
          if($r){while($row = sqlsrv_fetch_array($r)){
            $image = imagecreatefromstring(base64_decode($row['Image']));
            $width = imagesx($image);
            ...
            <div class='row'>
              ...
              <div class='col-xs-8'><?php ?><img width='100%' src="<?php print "data:" . $row['Image_Type'] . ";base64, " . base64_encode($rawImageBytes);?>" /></div>
            </div>
          <?php }}?>
          <div class='row'>
          ...
          <div class='row'>
            ...
            <div class='col-xs-8'><input type='text' pattern='[0123456789]*' placeholder='Length' name='Length' value='<?php echo isset($Item['Length']) ? $Item['Length'] : '';?>' /></div>
          </div>
          <div class='row'>
            <div class='col-xs-4'>Width:</div>
            <div class='col-xs-8'><input type='text' pattern='[0123456789]*' placeholder='Width' name='Width' value='<?php echo isset($Item['Width']) ? $Item['Width'] : '';?>' /></div>
          </div>
          <div class='row'>
            <div class='col-xs-4'>Height:</div>
            <div class='col-xs-8'><input type='text' pattern='[0123456789]*' placeholder='Height' name='Height' value='<?php echo isset($Item['Height']) ? $Item['Height'] : '';?>' /></div>
          </div>
          <div class='row'>
            <div class='col-xs-4'>Wattage Rating:</div>
            <div class='col-xs-8'><input type='text' pattern='[0123456789]*' placeholder='Wattage Rating' name='Wattage_Rating' value='<?php echo isset($Generator['Wattage_Rating']) ? $Generator['Wattage_Rating'] : '';?>' /></div>
          </div>
          <div class='row'>
            <div class='col-xs-4'>Power Input:</div>
            <div class='col-xs-8'><select name='Power_Input'>
              <option value=''>Select</option>
              <option <?php echo $Generator['Power_Input'] == 'Alternating Current (A.C.)' ? 'selected' : '';?> value='Alternating Current (A.C.)'>Alternating Current (A.C.)</option>
              <option <?php echo $Generator['Power_Input'] == 'Direct Current (D.C.)' ? 'selected' : '';?> value='Direct Current (D.C.)'>Direct Current (D.C.)</option>
            </select></div>
          </div>
          <div class='row'>
            <div class='col-xs-4'>Input Voltage:</div>
            <div class='col-xs-8'><input type='text' pattern='[0123456789]*' placeholder='Voltage' name='Input_Voltage' value='<?php echo isset($Generator['Input_Voltage']) ? $Generator['Input_Voltage'] : '';?>' /></div>
          </div>
          <div class='row'>
            <div class='col-xs-4'>Input Amperage:</div>
            <div class='col-xs-8'><input type='text' pattern='[0123456789]*' placeholder='Amperage' name='Input_Amperage' value='<?php echo isset($Generator['Input_Amperage']) ? $Generator['Input_Amperage'] : '';?>' /></div>
          </div>
          <div class='row'>
            <div class='col-xs-4'>Power Output:</div>
            <div class='col-xs-8'><select name='Power_Output'>
              <option value=''>Select</option>
              <option <?php echo $Generator['Power_Output'] == 'Alternating Current (A.C.)' ? 'selected' : '';?> value='Alternating Current (A.C.)'>Alternating Current (A.C.)</option>
              <option <?php echo $Generator['Power_Output'] == 'Direct Current (D.C.)' ? 'selected' : '';?> value='Direct Current (D.C.)'>Direct Current (D.C.)</option>
            </select></div>
          </div>
          <div class='row'>
            <div class='col-xs-4'>Output Voltage:</div>
            <div class='col-xs-8'><input type='text' pattern='[0123456789]*' placeholder='Voltage' name='Output_Voltage' value='<?php echo isset($Generator['Output_Voltage']) ? $Generator['Output_Voltage'] : '';?>' /></div>
          </div>
          <div class='row'>
            <div class='col-xs-4'>Output Amperage:</div>
            <div class='col-xs-8'><input type='text' pattern='[0123456789]*' placeholder='Amperage' name='Output_Amperage' value='<?php echo isset($Generator['Output_Amperage']) ? $Generator['Output_Amperage'] : '';?>' /></div>
          </div>
          <div class='row'>
            <div class='col-xs-4'>RPM:</div>
            <div class='col-xs-8'><input type='text' pattern='[0123456789]*' placeholder='RPM' name='RPM' value='<?php echo isset($Generator['RPM']) ? $Generator['RPM'] : '';?>' /></div>
          </div>
          <div class='row'>
            <div class='col-xs-4'>Horsepower:</div>
            <div class='col-xs-8'><input type='text' pattern='[0123456789]*' placeholder='Horsepower' name='Horsepower' value='<?php echo isset($Generator['Horsepower']) ? $Generator['Horsepower'] : '';?>' /></div>
          </div>
          <div class='row'>
            <div class='col-xs-4'>Windings:</div>
            <div class='col-xs-8'><textarea rows='5' style='width:100%;' name='Windings'><?php echo isset($Generator['Windings']) ? $Generator['Windings'] : '';?></textarea></div>
          </div>
          <div class='row'><div class='col-xs-12'>&nbsp;</div></div>
          <div class='row'><div class='col-xs-12'><button onClick="save_Generator(this);" type='button' style='width:100%;height:42px;'>Save</button></div></div>
          <div class='row'><div class='col-xs-12'>&nbsp;</div></div>
          <div class='row'><div class='col-xs-12'>&nbsp;</div></div>
        </form></div>
        <script>
        function save_Generator(link){
          $(link).html("Saving <img src='media/images/spinner.gif' height='25px' width='auto' />");
          $(link).attr('disabled','disabled');
          var formElement = document.getElementById('form_Generator');
          var formData = new FormData(formElement);
          formData.append('ID', '<?php echo $_GET['ID'];?>');
          if($("#form_Generator input:invalid").length == 0){
            $.ajax({
              url:"cgi-bin/php/post/unit/generator.php",
              data:formData,
              processData: false,
              contentType: false,
              timeout:15000,
        ...
        <div class='panel-body'>
          <div class='row'>
            <div class='Home-Screen-Option col-lg-1 col-md-2 col-xs-3' onclick="someFunction(this,'unit-generator-motor.php?ID=<?php echo $_GET['ID'];?>');">
              <div class='nav-icon'><img src='media/images/icons/motor.png' width='auto' height='35px' /></div>
              <div class ='nav-text'>Motor</div>
            </div>
            <div class='Home-Screen-Option col-lg-1 col-md-2 col-xs-3' onclick="someFunction(this,'unit-generator-motor-armature.php?ID=<?php echo $_GET['ID'];?>');">
              <div class='nav-icon'><img src='media/images/icons/armature.png' width='auto' height='35px' /></div>
              <div class ='nav-text'>Armature</div>
            </div>
            <div class='Home-Screen-Option col-lg-1 col-md-2 col-xs-3' onclick="someFunction(this,'unit-generator-motor-armature-interpoles.php?ID=<?php echo $_GET['ID'];?>');">
