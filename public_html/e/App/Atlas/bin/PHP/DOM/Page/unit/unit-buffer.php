...
if(isset($_SESSION['User'],$_SESSION['Hash'])){
  ...
  if($r){
    ...
    if($r){
      ...
      $sQuery = "SELECT Buffer.* FROM Device.dbo.Buffer WHERE Buffer.Item = ?";
      $params = array($Item['ID']);
      $r = sqlsrv_query($database_Device, $sQuery, $params);
      if($r){
        $Buffer = sqlsrv_fetch_array($r);
      }
    }
  }
?><!DOCTYPE html>
			<div class="panel panel-primary">
        <div class='panel-heading' onClick="someFunction(this,'unit-buffer.php?ID=<?php echo $_GET['ID'];?>');">
          <img src='media/images/icons/buffer.png' width='auto' height='35px' />
          <span>Buffer</span>
        </div>
				<div class='panel-body' style='padding-top:10px;'><form id='form_Buffer'>
          <?php if(isset($Item)){?><input type='hidden' value='<?php echo $Item['ID'];?>' name='Item' /><?php }?>
          <div class='row'>
            <div class='col-xs-4'>Location:</div>
            <div class='col-xs-8' onclick="someFunction(this,'unit-pit.php?ID=<?php echo $_GET['ID'];?>');"><img src='media/images/icons/pit.png' width='auto' height='25px' /> Pit</div>
          </div>
          <div class='row'>
          ...
          <div class='row'>
            ...
            <div class='col-xs-8'><select name='Type'>
              <option value=''>Select</option>
              <option <?php echo $Buffer['Type'] == 'Oil' ? 'selected' : '';?> value='Oil'>Oil</option>
              <option <?php echo $Buffer['Type'] == 'Spring' ? 'selected' : '';?> value='Spring'>Spring</option>
              <option <?php echo $Buffer['Type'] == 'Polyurethane' ? 'selected' : '';?> value='Polyurethane'>Polyurethane</option>
            </select></div>
          </div>
          <div class='row'>
            <div class='col-xs-4'>Stroke Range:</div>
            <div class='col-xs-8'><input placeholder='Stroke Range' pattern='[0123456789]*' type='text' value='<?php echo isset($Buffer['Stroke_Range']) ? $Buffer['Stroke_Range'] : '';?>' name='Stroke_Range' /></div>
          </div>
          <div class='row'>
            <div class='col-xs-4'>Maximum Car Speed:</div>
            <div class='col-xs-8'><input placeholder='Maximum Car Speed' pattern='[0123456789]*' type='text' value='<?php echo isset($Buffer['Maximum_Car_Speed']) ? $Buffer['Maximum_Car_Speed'] : '';?>' name='Maximum_Car_Speed' /></div>
          </div>
          <div class='row'>
            <div class='col-xs-4'>Load:</div>
            <div class='col-xs-3'><input placeholder='min' pattern='[0123456789]*' type='text' value='<?php echo isset($Buffer['Minimum_Load']) ? $Buffer['Minimum_Load'] : '';?>' name='Minimum_Load' size='3' /></div>
            <div class='col-xs-2'>to</div>
            <div class='col-xs-3'><input placeholder='max' pattern='[0123456789]*' type='text' value='<?php echo isset($Buffer['Maximum_Load']) ? $Buffer['Maximum_Load'] : '';?>' name='Maximum_Load' size='3' /></div>
          </div>
          <div class='row'><div class='col-xs-12'>&nbsp;</div></div>
          <div class='row'><div class='col-xs-12'>&nbsp;</div></div>
          <div class='row'><div class='col-xs-12'><button onClick='save_Buffer(this);' type='button' style='width:100%;height:42px;'>Save</button></div></div>
          <div class='row'><div class='col-xs-12'>&nbsp;</div></div>
          <div class='row'><div class='col-xs-12'>&nbsp;</div></div>
        </form></div>
        <script>
          function save_Buffer(link){
            $(link).html("Saving <img src='media/images/spinner.gif' height='25px' width='auto' />");
            $(link).attr('disabled','disabled');
            var formElement = document.getElementById('form_Buffer');
            var formData = new FormData(formElement);
            formData.append('ID', '<?php echo $_GET['ID'];?>');
            if($("#form_Buffer input:invalid").length == 0){
              $.ajax({
                url:"cgi-bin/php/post/unit/buffer.php",
                data:formData,
                processData: false,
                contentType: false,
                timeout:15000,
        ...
        <div class='panel-body'>
          <div class='row'>
            <div class='Home-Screen-Option col-lg-1 col-md-2 col-xs-3' onclick="someFunction(this,'unit-strike_pad.php?ID=<?php echo $_GET['ID'];?>');">
              <div class='nav-icon'><img src='media/images/icons/strike_pad.png' width='auto' height='35px' /></div>
              <div class ='nav-text'>Strike Pad</div>
            </div>
            <div class='Home-Screen-Option col-lg-1 col-md-2 col-xs-3' onclick="someFunction(this,'unit-buffer-piston.php?ID=<?php echo $_GET['ID'];?>');">
              <div class='nav-icon'><img src='media/images/icons/buffer_piston.png' width='auto' height='35px' /></div>
              <div class ='nav-text'>Piston</div>
            </div>
            <div class='Home-Screen-Option col-lg-1 col-md-2 col-xs-3' onclick="someFunction(this,'unit-buffer-spring.php?ID=<?php echo $_GET['ID'];?>');">
              <div class='nav-icon'><img src='media/images/icons/buffer_spring.png' width='auto' height='35px' /></div>
              <div class ='nav-text'>Spring</div>
            </div>
            <div class='Home-Screen-Option col-lg-1 col-md-2 col-xs-3' onclick="someFunction(this,'unit-buffer-base_plate.php?ID=<?php echo $_GET['ID'];?>');">
              <div class='nav-icon'><img src='media/images/icons/base_plate.png' width='auto' height='35px' /></div>
              <div class ='nav-text'>Base Plate</div>
            </div>
          </div>
        </div>-->
			</div>
