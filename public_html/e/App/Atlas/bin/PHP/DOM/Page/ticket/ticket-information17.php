...
if(isset($_SESSION['User'],$_SESSION['Hash'])){
    ...
    sqlsrv_query($Portal,
      " INSERT INTO Activity([User], [Date], [Page])
        VALUES(?,?,?);",array( $_SESSION['User'], date('Y-m-d H:i:s'), 'ticket.php' ));
    $r = sqlsrv_query($NEI,
      " SELECT Emp.*,
              fFirst AS First_Name,
              Last as Last_Name
        FROM Emp WHERE ID = ?;", array( $_SESSION['User'] ));
    ...
    if( isset($My_Privileges['Ticket'])
        ...
        && $My_Privileges['Location']['Other_Privilege'] >= 4){$Privileged = TRUE;}
    elseif($My_Privileges['Ticket']['Group_Privilege'] >= 4){
      $r = sqlsrv_query($NEI,
        " SELECT  Tickets.Location
          FROM    (
            (SELECT TicketO.LID AS Location FROM nei.dbo.TicketO)
            UNION ALL
            (SELECT TicketD.Loc AS Location FROM nei.dbo.TicketD)
            UNION ALL
            (SELECT TicketDArchive.Loc AS Location FROM nei.dbo.TicketDArchive)
          ) AS Tickets
          WHERE Tickets.Location = ?
        ;", array( $_GET['ID'] ));
        if($r){$Location = sqlsrv_fetch_array($r)['Location'];}
        if(!is_null($Location)){
          $r = sqlsrv_query($NEI,
          " SELECT  Tickets.*
            FROM    (
                      (SELECT TicketO.fWork AS Employee_Work_ID, TicketO.LID AS Location FROM nei.dbo.TicketO)
                      UNION ALL
                      (SELECT TicketD.fWork AS Employee_Work_ID, TicketD.Loc AS Location FROM nei.dbo.TicketD)
                      UNION ALL
                      (SELECT TicketDArchive.fWork AS Employee_Work_ID, TicketDArchive.Loc AS Location FROM nei.dbo.TicketDArchive)
            ) AS Tickets
            WHERE   Tickets.Location = ?
                    AND Tickets.Employee_Work_ID = ?
            ;", array($Location, $My_User['fWork']));
            if($r){
                if($r){$a = sqlsrv_fetch_array($r);}
                if(!is_null($a)){$Privileged = true;}
            }
        }
        if(!$Privileged){
    ...
    if(!isset($array['ID'])  || !$Privileged){}
      ...
      $r = sqlsrv_query($NEI,
        " SELECT  TicketO.*,
                  ...
                  'TicketO'           AS Table2,
                  [Location].GPS      AS Geofence,
                  [Location].Distance AS Distance
				FROM      nei.dbo.TicketO
                  ...
                  LEFT JOIN Portal.dbo.[Location] ON TicketO.LID      = [Location].Total_Service_ID
            WHERE TicketO.ID = ?;", array( $_GET['ID'] ));
        ...
        $r = sqlsrv_query($NEI,
          " SELECT  TicketD.*,
            ...
            FROM    nei.dbo.TicketD
                    ...
                    LEFT JOIN Portal.dbo.[Location] ON TicketD.Loc      = [Location].Total_Service_ID
            WHERE   TicketD.ID = ?;", array( $_GET['ID'] ));
        $Ticket = sqlsrv_fetch_array($r);
    }
    if($Ticket['ID'] == "" || $Ticket['ID'] == 0 || !isset($Ticket['ID'])){
        $r = sqlsrv_query($NEI,
          " SELECT  TicketDArchive.*,
                    Loc.Tag             AS Tag,
                    Loc.Loc             AS Location_ID,
                    Loc.Address         AS Address,
            ...
            FROM    nei.dbo.TicketDArchive
                    ...
                    LEFT JOIN Portal.dbo.[Location] ON TicketDArchive.Loc = [Location].Total_Service_ID
            WHERE   TicketDArchive.ID = ?;", array( $_GET['ID'] ));
        $Ticket = sqlsrv_fetch_array($r);
    }
...
<div class="panel panel-primary">
  ...
  if($A_Latitude != 0 && $A_Longitude != 0 && $M_Latitude != 0 && $M_Longitude != 0){?>
    ...
    /*function renderMap(){
        ...
        var myOptions = {
          zoom: <?php /*echo isset($_GET['Latitude'], $_GET['Longitude']) ? 18 : 10;*/?>18,
          center: latlng
        };
  ...
  <?php }?>
  <?php
    $Tasks = (strpos($Ticket['fDesc'], ';')) !== false ? explode(";", substr($Ticket['fDesc'], 0, strlen($Ticket['fDesc']) - 1)) : array($Ticket['fDesc']);
    ...
    <div class='panel-body' style='background-color:#1d1d1d !important;'>
      <?php
        $count = 0;
        $count3 = 0;
        if(count($Tasks) > 0 && count($Deficiences) == 0){
          foreach($Tasks AS $index=>$Task){
            if(strpos($Task, 'Scheduled for') !== False){continue;}
            $count2 = 0;
            foreach($SQL_Tasks_Descriptions as $index2=>$Description) {
              ...
              if((trim($Task) == $Description && ($SQL_Tasks_Percentages[$count2] == '1'))){
                ...
                ?><div class='row' style='background-color:#2d2d2d !important;margin-top:5px;padding-top:10px;padding-bottom:10px;'>
                  ...
                  <div class='col-xs-2'><input id='task_index_<?php echo $count3;?>' disabled checked type='checkbox' class='Task_Checkbox' onClick='toggleTaskResolution(this)' name='Task[<?php echo $count;?>]'  style='width:100%;height:25px;' /></div>
                </div><?php
                continue 2;
              }
              $count2++;
            ...
            <div class='row' style='background-color:#2d2d2d !important;margin-top:5px;padding-top:10px;padding-bottom:10px;'>
              <div class='col-xs-1'><?php echo $count;?></div>
              <div class='col-xs-9 Task_Description'><?php echo trim($Task);?></div>
              <div class='col-xs-2'><input  style='width:100%;height:25px;' id='task_index_<?php echo $count3;?>' <?php if($Ticket['Assigned'] > 3 || $Ticket['Assigned'] == 1){?>disabled<?php }?> type='checkbox' class='Task_Checkbox' onChange='toggleTaskResolution(this)' name='Task[<?php echo $count;?>]' /></div>
            </div>
          <?php
        }
      }?>
  ...
  <?php ?>
  <script>
  //$(document).ready(function(){toggleContinueWorked('.Task_Checkbox, .Deficiency_Checkbox');});
  function toggleContinueWorked(className){toggleContinuedWork(checkerBoxes(className));}
  <?php /*function checkBoxed(el){
    return el.checked;
  }*/?>
  function toggleContinuedWork(state){document.getElementById('Continue_Work').checked = state;}
  function checkerBoxes(className){
    boolean = false;
    $(className).each(function(){
      if(this.checked == false){boolean = true;}
    });
    return boolean;
  }
  ...
  function toggleTaskResolution(link){
    toggleContinueWorked('.Task_Checkbox');
    if(link.checked){
  ...
  <div class='panel-body'>
    ...
    function add_Deficiency_Resolution(link){
      toggleContinueWorked('.Deficiency_Checkbox');
      $("textarea[name='Resolution']").val($("textarea[name='Resolution']").val().length > 0 ?
        html_unescape($("textarea[name='Resolution']").val()) + '\n' + 'Completed: ' + html_unescape($(link).parent().prev().html()) :
        'Completed: ' + html_unescape($(link).parent().prev().html()));
    }
  ...
  <div class='panel-body' style='padding:10px;'>
    ...
    <div class='row'>
      ...
      <div class='col-xs-12'>&nbsp;</div>
      <div class='col-xs-12'><textarea disabled id='TextareaResolution' name='Resolution' style='width:100%;' rows='5' placeholder='Resolution Description'><?php echo $Ticket2['DescRes'];?></textarea></div>
    ...
    <div class='row resolution'>
      ...
      <div class='col-xs-8'><input id='Continue_Work' type='checkbox' value='1' name='Continue_Work' style='height:15px;width:15px;' /></div>
      <div class='col-xs-12'>&nbsp;</div>
    </div>
    <div class='row'><div class='col-xs-12'>&nbsp;</div></div>
  </div>
  ...
  <div class='panel-body' style='padding:10px;'>
    ...
    <div class='row'><div class='col-xs-12'>&nbsp;</div></div>
    <div class='row'>
      <div class='col-xs-4'><i class="fa fa-paragraph fa-1x fa-fw" aria-hidden="true"></i> Problem:</div>
      <div class='col-xs-8'><select name='Resolution_Code[0]' style='width:100%;'>
        <?php
            $Codes = array(
                'Shutdown',
                'Door',
            ...
            foreach($Codes as $Code){
                ?><option value='<?php echo $Code;?>'><?php echo $Code;?></option><?php
            }
            ?>
      </select></div>
      <div class='col-xs-12'>&nbsp;</div>
      <div class='col-xs-4'><i class="fa fa-paragraph fa-1x fa-fw" aria-hidden="true"></i> Problem:</div>
      <div class='col-xs-8'><select name='Resolution_Code[1]' style='width:100%;'>
        <?php
            $Codes = array(
              'Car',
              'Machine Room',
      ...
      <div class='col-xs-12'>&nbsp;</div>
      <div class='col-xs-4'><i class="fa fa-paragraph fa-1x fa-fw" aria-hidden="true"></i> Area:</div>
      <div class='col-xs-8'><select name='Resolution_Code[2]' style='width:100%;'>
        <?php
            $Resolution_Codes = array(
                'LSD - Left shutdown',
                'FTS - Further troubleshoot',
                'D/T - correct door trouble',
                'M/S - Misuse',
                'C/T - Cartop',
                'ROA - Running on arrival',
                'SIG. - Signal issue resolved',
                'R/S - Reset',
                'GEN. - correct generator issue',
                'HT/MT - correct Hoist Motor issue',
                'LUBE - Lubrication performed',
                'R/E - correct issue door reversing edge',
                'RPT - release people trapped',
                'POA - People out on arrival',
                'N/A - system check nothing apparent',
                'DEBRIS - Debris found and removed',
                'C/I - correct controller issue',
                'M/B - Adjust Machine brake',
                'R/G - reset governor',
                'R/S - reset safety',
                'RTS - Return to service',
                'COMM. - correct communication issue',
                'C/A - clean and adjust',
                'R/F - replace fuse',
                'M/L - Mainline disconnect switch'
            );
            foreach($Resolution_Codes as $Resolution_Code){
                ?><option value='<?php echo $Resolution_Code;?>'><?php echo $Resolution_Code;?></option><?php
            }
            ?>
      </select></div>
      <div class='col-xs-12'>&nbsp;</div>
      <div class='col-xs-12'><button onClick='addResolutionCode();'>Add Resolution Code</button></div>
      <script>
        function addResolutionCode(){
            if($("#TextareaResolution").val() == ''){
                $("#TextareaResolution").val('Problem: ' + $("select[name='Resolution_Code[0]']").val() + '\nArea: ' + $("select[name='Resolution_Code[1]']").val() + '\nResolution: ' + $("select[name='Resolution_Code[2]']").val());
            } else {
                $("#TextareaResolution").val($("#TextareaResolution").val() + "\n--------------------------\n" + 'Problem: ' + $("select[name='Resolution_Code[0]']").val() + '\nArea: ' + $("select[name='Resolution_Code[1]']").val() + '\nResolution: ' + $("select[name='Resolution_Code[2]']").val());
            }
        }
      </script>
      <div class='col-xs-12'>&nbsp;</div>
      <div class='col-xs-12'><textarea disabled id='TextareaResolution' name='Resolution' style='width:100%;' rows='5' placeholder='Resolution Description'></textarea></div>
    </div>
    <div class='row'><div class='col-xs-12'>&nbsp;</div></div>
  </div>
  <div class='panel-heading' style='padding:1px;padding-left:10px !important;margin:0px;margin-bottom:5px;'><h4><i class="fa fa-pencil fa-1x fa-fw" aria-hidden="true"></i> Comments</h4></div>
  <div class='panel-body' style='padding:10px;'>
    <div class='col-xs-12'>&nbsp;</div>
    <div class='col-xs-12'><textarea name='Internal_Comments' placeholder='Internal Comments' style='width:100%;' rows='5'></textarea></div>
    <div class='col-xs-12'>&nbsp;</div>
  </div>
  <div class='panel-heading' style='padding:1px;padding-left:10px !important;margin:0px;margin-bottom:5px;'><h4><i class="fa fa-pencil fa-1x fa-fw" aria-hidden="true"></i> Signature</h4></div>
  <div class='panel-body' style='padding:10px;'>
...
<?php require("../../../../cgi-bin/js/pages/ticket15.php");?>
<style>
