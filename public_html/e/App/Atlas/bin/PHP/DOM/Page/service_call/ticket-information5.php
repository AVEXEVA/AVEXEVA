    ...
    if($r){
      ...
      if(is_array($Ticket)){
        ...
        if(isset($Ticket['ID']) && is_numeric($Ticket['ID']) && isset($Ticket['WorkOrder']) && ($Ticket['WorkOrder'] == null || $Ticket['WorkOrder'] == 0 || $Ticket['WorkOrder'] == '')){
          sqlsrv_query($NEI,"UPDATE nei.dbo.TicketO SET TicketO.WorkOrder = ? WHERE TicketO.ID = ?",array($_GET['ID'], $_GET['ID']));
...
if($_SESSION['User'] == '895'){
  $r = sqlsrv_query($NEI,"SELECT ID, Name FROM Portal.dbo.Category_Elevator_Part;");
  $Elevator_Parts = array();
  if($r){while($row = sqlsrv_fetch_array($r)){
    $Elevator_Parts[$row['ID']] = str_replace("/","\/",$row['Name']);
  }}
  $r = sqlsrv_query($NEI,"SELECT ID, Name FROM Portal.dbo.Category_Remedy;");
  $Remedies = array();
  if($r){while($row = sqlsrv_fetch_array($r)){
    $Remedies[$row['ID']] = str_replace("/","\/",$row['Name']);
  }}
  $r = sqlsrv_query($NEI,"SELECT ID, Name FROM Portal.dbo.Category_Violation_Condition;");
  $Conditions = array();
  if($r){while($row = sqlsrv_fetch_array($r)){
  ...
  if(isset($Ticket['Job_Remarks'],$Ticket['Level']) && $Ticket['Level'] == 4 && strlen($Ticket['Job_Remarks']) > 0){
    ...
    if(is_array($matches) && count($matches) > 0){
      ...
      while($check == True){
        ...
        $r = sqlsrv_query($NEI,"SELECT * FROM Portal.dbo.Deficiency WHERE Deficiency.Job = ? AND Deficiency.Elevator_Part = ? AND Deficiency.Remedy = ? AND Deficiency.Violation = ?;",array($Ticket['Job_ID'],$Elevator_Part,$Remedy,$Condition));
        if(!$r || ($r && !is_array(sqlsrv_fetch_array($r)))){
          sqlsrv_query($NEI,"INSERT INTO Portal.dbo.Deficiency(Job, Elevator_Part, Remedy, Violation, Comments, [Percentage]) VALUES(?, ?, ?, ?, ?, ?);",array($Ticket['Job_ID'],$Elevator_Part,$Remedy,$Condition,$Comments,0));
        }
        $i++;
...
<div class="panel panel-primary">
  <div class='panel-heading' style='padding:1px;padding-left:10px !important;margin:0px;margin-bottom:5px;'><h4><?php $Icons->Info(1);?> Description</h4></div>
    ...
    <div class='row'>
      <div class='col-xs-4'><?php $Icons->Blank(1);?> Sub-Type:</div>
      <div class='col-xs-8'><?php
        $Levels = array(
          1=>'Service Call',
          2=>'Trucking',
    ...
    <div class='row'>
      <div class='col-xs-4'><?php $Icons->Blank(1);?> Priority:</div>
      <div class='col-xs-8'><?php
  ...
  $r = sqlsrv_query($NEI,
    " SELECT  Deficiency.ID AS ID,
      ...
      WHERE   Violation.Job = ?;",array($Ticket['Job']));
  ...
  if($Ticket['Level'] == 10){
    ...
    ?><div class='panel-heading' style='padding:1px;padding-left:10px !important;margin:0px;margin-bottom:5px;'><h4><?php $Icons->Ticket(1);?> Prevenative Maintenance Tasks</h4></div>
    <div class='panel-body' style='background-color:#1d1d1d !important;'><?php
    $r = sqlsrv_query($NEI,"SELECT * FROM Portal.dbo.Preventative_Maintenance_Abstract;");
    if($r){while($row = sqlsrv_fetch_array($r)){?><div class='row'>
      ...
      <div class='col-xs-2'><input type='checkbox'
        ...
        <?php
        $resource = sqlsrv_query($NEI,"SELECT * FROM Portal.dbo.Preventative_Maintenance WHERE Preventative_Maintenance.Unit = ? AND Preventative_Maintenance.Date >= ?",array($Ticket['Unit_ID'],date("Y-m-d H:i:s",strtotime("-1 month"))));
        if($resource && is_array(sqlsrv_fetch_array($resource))){?>checked<?php }?> /></div>
    </div><?php }}
    ?></div><?php
  }?>
  <div class='panel-heading' style='padding:1px;padding-left:10px !important;margin:0px;margin-bottom:5px;'><h4><?php $Icons->Ticket(1);?> Work Order</h4></div>
  <div class='panel-body'>
    ...
    <div class='row'>
      <div class='col-xs-4'><?php $Icons->Info(1);?> Grouping:</div>
      <?php
      if(isset($_GET['Edit']) || ($Ticket['Assigned'] >= 1 && $Ticket['Assigned'] <= 3)){?>
  ...
  <div class='panel-body' style='padding:10px;'>
    ...
    <div class='row chargeable_Image <?php echo isset($Ticket2['Charge']) && $Ticket2['Charge'] == 1 ? 'active' : '';?>'>
      <div class='col-xs-4'><?php $Icons->Blank();?> Picture:</div>
  ...
  <div class='panel-body' style='padding:10px;font-size:14px;'>
    ...
    <div class='row expenses'>
      <div class='col-xs-4'><i class="fa fa-paperclip fa-1x fa-fw" aria-hidden="true"></i> Photo:</div>
      <?php
        $r = sqlsrv_query($Portal,"SELECT [TicketPic].[PicData] FROM nei.dbo.TicketPic WHERE [TicketPic].TicketID = ? AND [TicketPic].PictureName LIKE '%TCK%';",array($_GET['ID']));
        $i = 0;
        if($r){while($row = sqlsrv_fetch_array($r)){
          $row['Type'] = 'image/jpeg';
  ...
  <div class='panel-body' style='padding:10px;font-size:14px;'>
    ...
    <div class='row resolution'>
      ...
      <?php
        $r = sqlsrv_query($Portal,"SELECT [TicketPic].[PicData] FROM nei.dbo.TicketPic WHERE [TicketPic].TicketID = ? AND [TicketPic].PictureName LIKE '%TIK%';",array($_GET['ID']));
        $i = 0;
        if($r){while($row = sqlsrv_fetch_array($r)){
          $row['Type'] = 'image/jpeg';
          if($i > 0){?><div class='col-xs-4'>&nbsp;</div><?php }
...
<?php require("../../../../cgi-bin/js/pages/ticket5.php");?>
<style>
