    ...
    if(!isset($array['ID'])  || !$Privileged || !is_numeric($_GET['ID'])){?><html><head><script>document.location.href="../login.php?Forward=unit<?php echo (!isset($_GET['ID']) || !is_numeric($_GET['ID'])) ? "s.php" : ".php?ID={$_GET['ID']}";?>";</script></head></html><?php }
        ...
        if($r){
          ...
          if($r){
            ...
            $sQuery = "SELECT Starter.* FROM Device.dbo.Starter WHERE Starter.Item = ?";
            $params = array($Item['ID']);
            $r = sqlsrv_query($database_Device, $sQuery, $params);
            if($r){
              $Starter = sqlsrv_fetch_array($r);
            }
          }
        }
?><!DOCTYPE html>
<div class="panel panel-primary">
  <div class='panel-heading' onclick="someFunction(this,'unit-starter.php?ID=<?php echo $_GET['ID'];?>');"><img src='media/images/icons/starter.png' width='auto' height='35px' /> Starter</div>
	<div class='panel-body' style='margin-top:10px;'><form id='form_Starter'>
    ...
    <div class='row'>
      ...
      <div class='col-xs-8'><input type='text' placeholder='Product Name' value='<?php echo isset($Product['Name']) ? $Product['Name'] : '';?>' name='Product' /></div>
    </div>
    <div class='row'>
    ...
    <div class='row'>
      ...
      <div class='col-xs-8'><select name='Condition' style='color:black;'>
    ...
    <div class='row'>
      ...
      <div class='col-xs-8'><input type='text' placeholder='Horsepower' value='<?php echo isset($Starter['Horsepower']) ? $Starter['Horsepower'] : '';?>' name='Horsepower' /></div>
    </div>
    <div class='row'>
      <div class='col-xs-4'>Soft Starter:</div>
      <div class='col-xs-8'><select style='color:black;'name='Soft_Starter'>
        <option value=''>Select</option>
        <option <?php echo $Starter['Soft_Starter'] == '0' ? 'selected' : '';?> value='0'>No</option>
        <option <?php echo $Starter['Soft_Starter'] == '1' ? 'selected' : '';?> value='1'>Yes</option>
      </select></div>
    </div>
    <div class='row'>
      <div class='col-xs-4'>Over Voltage Protection:</div>
      <div class='col-xs-8'><select style='color:black;'name='Over_Voltage_Protection'>
        <option value=''>Select</option>
        <option <?php echo $Starter['Over_Voltage_Protection'] == '0' ? 'selected' : '';?> value='0'>No</option>
        <option <?php echo $Starter['Over_Voltage_Protection'] == '1' ? 'selected' : '';?> value='1'>Yes</option>
      </select></div>
    </div>
    <div class='row'>
      <div class='col-xs-4'>Under Voltage Protection:</div>
      <div class='col-xs-8'><select style='color:black;'name='Under_Voltage_Protection'>
        <option value=''>Select</option>
        <option <?php echo $Starter['Under_Voltage_Protection'] == '0' ? 'selected' : '';?> value='0'>No</option>
        <option <?php echo $Starter['Under_Voltage_Protection'] == '1' ? 'selected' : '';?> value='1'>Yes</option>
      </select></div>
    </div>
  ...
  <script>
    function save_Starter(link){
      $(link).html("Saving <img src='media/images/spinner.gif' height='25px' width='auto' />");
      $(link).attr('disabled','disabled');
      var formElement = document.getElementById('form_Starter');
      var formData = new FormData(formElement);
      formData.append('ID', '<?php echo $_GET['ID'];?>');
      $.ajax({
        url:"cgi-bin/php/post/unit/starter.php",
        data:formData,
        processData: false,
        contentType: false,
        timeout:15000,
