...
if(isset($_SESSION['User'],$_SESSION['Hash'])){
  ...
  $params = array($_GET['ID'], 'Car_Station');
  ...
  if($r){
    ...
    if($r){
      ...
      $sQuery = "SELECT Car_Station.* FROM Device.dbo.Car_Station WHERE Car_Station.Item = ?";
      $params = array($Item['ID']);
      $r = sqlsrv_query($database_Device, $sQuery, $params);
      if($r){
        $Car_Station = sqlsrv_fetch_array($r);
      }
    }
  }
?><!DOCTYPE html>
			<div class="panel panel-primary">
        <div class='panel-heading' onClick="someFunction(this,'unit-car_station.php?ID=<?php echo $_GET['ID'];?>');"><img src='media/images/icons/car_station.png' width='auto' height='25px' /> Car Station Properties</div>
				<div class='panel-body' style='margin-top:10px;'><form id='form_Car_Station'>
          ...
          <div class='row'>
            <div class='col-xs-4'>No Smoking:</div>
            <div class='col-xs-8'><select name='No_Smoking'>
              <option value=''>Select</option>
              <option <?php echo $Car_Station['No_Smoking'] == 0 ? 'selected' : '';?> value='0'>No</option>
              <option <?php echo $Car_Station['No_Smoking'] == 1 ? 'selected' : '';?> value='1'>Yes</option>
            </select></div>
          </div>
          <div class='row'>
            <div class='col-xs-4'>Gong:</div>
            <div class='col-xs-8'><select name='Gong'>
              <option value=''>Select</option>
              <option <?php echo $Car_Station['Gong'] == 0 ? 'selected' : '';?> value='0'>No</option>
              <option <?php echo $Car_Station['Gong'] == 1 ? 'selected' : '';?> value='1'>Yes</option>
            </select></div>
          </div>
          <div class='row'>
            <div class='col-xs-4'>Handicap Chimes:</div>
            <div class='col-xs-8'><select name='Handicap_Chimes'>
              <option value=''>Select</option>
              <option <?php echo $Car_Station['Handicap_Chimes'] == 0 ? 'selected' : '';?> value='0'>No</option>
              <option <?php echo $Car_Station['Handicap_Chimes'] == 1 ? 'selected' : '';?> value='1'>Yes</option>
            </select></div>
          </div>
          <div class='row'>
            <div class='col-xs-4'>Electrical_Outlet:</div>
            <div class='col-xs-8'><select name='Electrical_Outlet'>
              <option value=''>Select</option>
              <option <?php echo $Car_Station['Electrical_Outlet'] == 0 ? 'selected' : '';?> value='0'>No</option>
              <option <?php echo $Car_Station['Electrical_Outlet'] == 1 ? 'selected' : '';?> value='1'>Yes</option>
            </select></div>
          </div>
          <div class='row'>
            <div class='col-xs-4'>Service_Cabinet:</div>
            <div class='col-xs-8'><select name='Service_Cabinet'>
              <option value=''>Select</option>
              <option <?php echo $Car_Station['Service_Cabinet'] == 0 ? 'selected' : '';?> value='0'>No</option>
              <option <?php echo $Car_Station['Service_Cabinet'] == 1 ? 'selected' : '';?> value='1'>Yes</option>
            </select></div>
          </div>
          <div class='row'>
            <div class='col-xs-4'>Intercom:</div>
            <div class='col-xs-8'><select name='Intercom'>
              <option value=''>Select</option>
              <option <?php echo $Car_Station['Intercom'] == 0 ? 'selected' : '';?> value='0'>No</option>
              <option <?php echo $Car_Station['Intercom'] == 1 ? 'selected' : '';?> value='1'>Yes</option>
            </select></div>
          </div>
          <div class='row'>
          ...
          <div class='row'><div class='col-xs-12'><button onClick='save_Car_Station(this);' type='button' style='width:100%;'>Save</button></div></div>
        </form></div></div>
        <script>
          function save_Car_Station(link){
            $(link).html("Saving <img src='media/images/spinner.gif' height='25px' width='auto' />");
            $(link).attr('disabled','disabled');
            var formElement = document.getElementById('form_Car_Station');
            var formData = new FormData(formElement);
            formData.append('ID', '<?php echo $_GET['ID'];?>');
            $.ajax({
              url:"cgi-bin/php/post/unit/car_station.php",
              data:formData,
              processData: false,
              contentType: false,
              timeout:15000,
        ...
        <div class='panel-body'>
          <div class='row'>
            <div class='Home-Screen-Option col-lg-1 col-md-2 col-xs-3' onclick="someFunction(this,'unit-car_station-buttons.php?ID=<?php echo $_GET['ID'];?>');">
              <div class='nav-icon'><img src='media/images/icons/button.png' width='auto' height='35px' /></div>
              <div class ='nav-text'>Buttons</div>
            </div>
            <div class='Home-Screen-Option col-lg-1 col-md-2 col-xs-3' onclick="someFunction(this,'unit-car_station-electrical_outlet.php?ID=<?php echo $_GET['ID'];?>');">
