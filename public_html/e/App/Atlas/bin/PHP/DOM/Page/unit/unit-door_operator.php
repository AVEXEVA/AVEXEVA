...
if(isset($_SESSION['User'],$_SESSION['Hash'])){
    ...
    if(!isset($array['ID'])  || !$Privileged || !is_numeric($_GET['ID'])){?><html><head><script>document.location.href="../login.php?Forward=unit<?php echo (!isset($_GET['ID']) || !is_numeric($_GET['ID'])) ? "s.php" : ".php?ID={$_GET['ID']}";?>";</script></head></html><?php }
        ...
        $params = array($_GET['ID'], 'Door_Operator');
        ...
        if($r){
          ...
          if($r){
            ...
            $sQuery = "SELECT Door_Operator.* FROM Device.dbo.Door_Operator WHERE Door_Operator.Item = ?";
            $params = array($Item['ID']);
            $r = sqlsrv_query($database_Device, $sQuery, $params);
            if($r){
              $Door_Operator = sqlsrv_fetch_array($r);
            }
          }
        }
?><!DOCTYPE html>
			<div class="panel panel-primary">
        <div class='panel-heading' onclick="someFunction(this,'unit-door_operator.php?ID=<?php echo $_GET['ID'];?>');"><img src='media/images/icons/door_operator.png' width='auto' height='35px' /> Door Operator</div>
				<div class='panel-body' style='padding-top:10px;'><form id='form_Door_Operator'>
          ...
          <div class='row'>
            ...
            <div class='col-xs-8'><select name='Speed' style='color:black;'>
              <option value=''>Select</option>
              <option <?php echo $Door_Operator['Speed'] == 'Single' ? 'selected' : '';?> value='Single'>Single</option>
              <option <?php echo $Door_Operator['Speed'] == 'Double' ? 'selected' : '';?> value='Double'>Double</option>
              <option <?php echo $Door_Operator['Speed'] == 'Peel-e' ? 'selected' : '';?> value='Peel-e'>Peel-e</option>
              <option <?php echo $Door_Operator['Speed'] == 'Center Parting' ? 'selected' : '';?> value='Center Parting'>Center Parting</option>
            </select></div>
          </div>
          <div class='row'>
            ...
            <div class='col-xs-8'><select name='Hand'>
              <option value=''>Select</option>
              <option value='Left' <?php echo $Door_Operator['Hand'] == 'Left' ? 'selected' : '';?>>Left</option>
              <option value='Right' <?php echo $Door_Operator['Hand'] == 'Right' ? 'selected' : '';?>>Right</option>
            </select></div>
          </div>
          <div class='row'><div class='col-xs-12'>&nbsp;</div></div>
          <div class='row'><div class='col-xs-12'>&nbsp;</div></div>
          <div class='row'><div class='col-xs-12'><button onClick="save_Door_Operator(this);" type='button' style='width:100%;height:42px;'>Save</button></div></div>
          <div class='row'><div class='col-xs-12'>&nbsp;</div></div>
          <div class='row'><div class='col-xs-12'>&nbsp;</div></div>
        </form></div>
        <script>
          function save_Door_Operator(link){
            $(link).html("Saving <img src='media/images/spinner.gif' height='25px' width='auto' />");
            $(link).attr('disabled','disabled');
            var formElement = document.getElementById('form_Door_Operator');
            var formData = new FormData(formElement);
            formData.append('ID', '<?php echo $_GET['ID'];?>');
            if($("#form_Door_Operator input:invalid").length == 0){
              $.ajax({
                url:"cgi-bin/php/post/unit/door_operator.php",
                data:formData,
                processData: false,
                contentType: false,
                timeout:15000,
        ...
        <!--<div class='panel-heading'>Configuration</div>
          ...
          <div class='row'>
            <div class='col-lg-1 col-md-2 col-xs-3' onclick="someFunction(this,'unit-door_operator-voice_box.php?ID=<?php echo $_GET['ID'];?>');">
              <div class='nav-icon'><img src='media/images/icons/voice_box.png' width='auto' height='35px' /></div>
              <div class ='nav-text'>Voice Box</div>
            </div>
            <div class='col-lg-1 col-md-2 col-xs-3' onclick="someFunction(this,'unit-door_operator-sheave.php?ID=<?php echo $_GET['ID'];?>');">
              <div class='nav-icon'><img src='media/images/icons/sheave.png' width='auto' height='35px' /></div>
              <div class ='nav-text'>Sheave</div>
            </div>
            <div class='col-lg-1 col-md-2 col-xs-3' onclick="someFunction(this,'unit-door_operator-boards.php?ID=<?php echo $_GET['ID'];?>');">
              <div class='nav-icon'><img src='media/images/icons/board.png' width='auto' height='35px' /></div>
              <div class ='nav-text'>Boards</div>
            </div>
            <div class='col-lg-1 col-md-2 col-xs-3' onclick="someFunction(this,'unit-door_operator-door_interlock.php?ID=<?php echo $_GET['ID'];?>');">
              <div class='nav-icon'><img src='media/images/icons/door_interlock.png' width='auto' height='35px' /></div>
              <div class ='nav-text'>Interlock</div>
            </div>
          </div>
        </div>-->
			</div>
