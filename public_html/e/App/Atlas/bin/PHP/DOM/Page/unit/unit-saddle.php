    ...
    if(!isset($array['ID'])  || !$Privileged || !is_numeric($_GET['ID'])){?><html><head><script>document.location.href="../login.php?Forward=unit<?php echo (!isset($_GET['ID']) || !is_numeric($_GET['ID'])) ? "s.php" : ".php?ID={$_GET['ID']}";?>";</script></head></html><?php }
        ...
        if($r){
          ...
          if($r){
            ...
            $sQuery = "SELECT Saddle.* FROM Device.dbo.Saddle WHERE Saddle.Item = ?";
            $params = array($Item['ID']);
            $r = sqlsrv_query($database_Device, $sQuery, $params);
            if($r){
              $Saddle = sqlsrv_fetch_array($r);
            }
          }
        }
?><!DOCTYPE html>
<div class="panel panel-primary">
  <div class='panel-heading' onClick="someFunction(this,'unit-saddle.php?ID=<?php echo $_GET['ID'];?>');"><img src='media/images/icons/saddle.png' width='auto' height='35px' /> Saddle</div>
	<div class='panel-body' style='margin-top:10px;'><form id='form_Saddle'>
    ...
    <div class='row'>
      ...
      <div class='col-xs-8'><input type='text' placeholder='Height' value='<?php echo isset($Saddle['Composition']) ? $Saddle['Composition'] : '';?>' name='Composition' /></div>
    </div>
    <div class='row'>
      <div class='col-xs-4'>Speed:</div>
      <div class='col-xs-8'><input type='text' placeholder='Speed' value='<?php echo isset($Saddle['Speed']) ? $Saddle['Speed'] : '';?>' name='Speed' /></div>
    </div>
    <div class='row'><div class='col-xs-12'>&nbsp;</div></div>
    <div class='row'><div class='col-xs-12'><button onClick='save_Saddle();' type='button' style='width:100%;'>Save</button></div></div>
  </form></div>
  <script>
    function save_Saddle(){
      var formElement = document.getElementById('form_Saddle');
      var formData = new FormData(formElement);
      formData.append('ID', '<?php echo $_GET['ID'];?>');
      $.ajax({
        url:"cgi-bin/php/post/unit/saddle.php",
        data:formData,
        processData: false,
        contentType: false,
        method:"POST",
