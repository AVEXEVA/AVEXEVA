...
if(isset($_SESSION['User'],$_SESSION['Hash'])){
    ...
    if(!isset($Connection['ID']) || !$Privileged){print json_encode(array('data'=>array()));}
        ...
        if($Mechanic > 0){
			...
			$r = sqlsrv_query($NEI,"
             ...
             Loc.Loc            AS Location_ID,
                ...
                TicketO.TimeRoute   AS En_Route,
						 ...
						 WHERE  TicketO.Assigned >= 2 AND TicketO.Assigned <= 3
						 		AND ((TicketO.DDate >= ? AND TicketO.DDate <= ?) OR (TicketO.EDate >= ? AND TicketO.EDate <= ?))
					) AS Tickets
			...
			",array(date('Y-m-d 00:00:00.000', strtotime('-1 days')), date('Y-m-d 00:00:00.000', strtotime('+1 days')),date('Y-m-d 00:00:00.000', strtotime('-1 days')), date('Y-m-d 00:00:00.000', strtotime('+1 days'))),array("Scrollable"=>SQLSRV_CURSOR_KEYSET));
      ...
      $levels = array(
    	...
    	if($r){
				while($i < $row_count){
                  ...
                  $Ticket['Action'] = $Ticket['Assigned'] == 2 ? 'En Route' : 'On Site';
                  $Ticket['Time'] = $Ticket['Assigned'] == 2 ? date('m/d/Y', strtotime($Ticket['Date'])) . ' ' . date('h:i A',strtotime($Ticket['En_Route'])) : date('m/d/Y', strtotime($Ticket['Date'])) . ' ' . date('h:i A',strtotime($Ticket['On_Site']));
						$data[] = $Ticket;
					}
					$i++;
				}
