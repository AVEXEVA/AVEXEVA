...
if(isset($_SESSION['User'],$_SESSION['Hash'])){
    ...
    if(!isset($array['ID'],$_GET['ID']) || !is_numeric($_GET['ID'])){?><html><head><script>document.location.href='../login.php';</script></head></html><?php }
		...
		$r = sqlsrv_query($NEI,"
			...
			WHERE 
				TicketO.Elev = ?
				AND (TicketO.fDesc   LIKE '%S/D%' 
					OR TicketO.fDesc LIKE '%SD%' 
					OR TicketO.Level = '1')
		;",array($_GET['ID']));
		...
		$r = sqlsrv_query($NEI,"
			...
			WHERE
				TicketD.Elev = ?
				AND (TicketD.fDesc   LIKE '%S/D%' 
					OR TicketD.fDesc LIKE '%SD%' 
					OR TicketD.Level = '1')
		...
		$r = sqlsrv_query($NEI,"
			...
			WHERE
				TicketDArchive.Elev = ?
				AND (TicketDArchive.fDesc   LIKE '%S/D%' 
					OR TicketDArchive.fDesc LIKE '%SD%' 
					OR TicketDArchive.Level = '1')
