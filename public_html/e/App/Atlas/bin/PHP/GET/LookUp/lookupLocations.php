...
$conn = sqlsrv_connect( $serverName, $connectionInfo);
     ...
     */
    $aColumns = array( 'Loc', 'Tag', 'ID', 'Maint', 'Latt', 'fLong', 'Remarks' );
	...
	$_GET['iDisplayLength'] = isset($_GET['length']) ? $_GET['length'] : 10;

    $Start = $_GET['iDisplayStart'];
    $Length = $_GET['iDisplayLength'];

    $End = intval($Start) + intval($Length);
	...
	$sWhere .= "Loc.Tag LIKE '%".addslashes($_GET['search']['value'])."%' OR ";
	$sWhere .= "Loc.ID LIKE '%".addslashes($_GET['search']['value'])."%' OR ";
    //$sWhere .= (strpos($_GET['search']['value'], 'maint') !== false) ? "Loc.Maint = 1 OR " : NULL;
	...
	if(TRUE || $My_Privileges['Location']['Other_Privilege'] >= 4){
		$sQuery = "
			SELECT *
			FROM (
				SELECT ROW_NUMBER() OVER (Order By Loc ASC) AS ROW_COUNT, *
				FROM   (SELECT " . str_replace(" , ", " ", implode(", ", $aColumns)) . " FROM $sTable $sWhere AND Loc.Status = 0) AS B
			) AS A
			WHERE A.ROW_COUNT > $Start AND A.ROW_COUNT <= $End
		";
		//echo $sQuery;
	} else {
		$sQuery = "
