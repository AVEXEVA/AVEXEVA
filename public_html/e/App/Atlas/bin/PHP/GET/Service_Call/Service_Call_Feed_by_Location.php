...
if(isset($_SESSION['User'],$_SESSION['Hash'])){
	...
	elseif($My_Privileges['Location']['User_Privilege'] >= 4 && is_numeric($_GET['ID'])){
		$r  = sqlsrv_query( $NEI,"SELECT * FROM nei.dbo.TicketO        WHERE TicketO.LID        = ? AND fWork=?;",array($_GET['ID'],$My_User['fWork']));
		$r2 = sqlsrv_query( $NEI,"SELECT * FROM nei.dbo.TicketD        WHERE TicketD.Loc        = ? AND fWork=?;",array($_GET['ID'],$My_User['fWork']));
		$r3 = sqlsrv_query( $NEI,"SELECT * FROM nei.dbo.TicketDArchive WHERE TicketDArchive.Loc = ? AND fWork=?;",array($_GET['ID'],$My_User['fWork']));
