...
if(isset($_SESSION['User'],$_SESSION['Hash'])){
    ...
    END)) IS NOT NULL THEN Job_Labor.[JOB #] ELSE 0 END
              ...
              LEFT JOIN (
                ...
                FROM      nei.dbo.OpenAR
                          LEFT JOIN nei.dbo.Invoice AS Inv ON OpenAR.Ref = Inv.Ref
                          LEFT JOIN nei.dbo.Job ON Inv.Job = Job.ID
                WHERE     Inv.fDate < GETDATE() - 150
                          AND (
                            (
                                (Job.Type    =   2 OR Job.Type = 3)
                                AND Job.Status  <>  0
              ...
              LEFT JOIN nei.dbo.Owner ON Owner.ID = Invoices.Owner OR Owner.ID = Labor.Owner OR Owner.ID = Materials.Owner
        ...
        WHERE (Invoices.Sum <> 0 OR Labor.Sum <> 0 OR Materials.Sum <> 0)
              ...
              {$Active_SQL}
        GROUP BY Job.Owner, Loc.Maint, Loc.Custom3, Rol.Name, Job.Loc, Loc.Tag, Terr.Name
      ) AS Locations
    ...
    $amount = 0;
      ...
      foreach($Customer as $key=>$value){
        $Customer[$key] = is_numeric($value) && !in_array($key,array('Cost_Margin', 'Profit_Percentage', 'Grade', 'Location_ID')) ? money_format('%.2n', $value) : $value;
      }
      $data[] = $Customer;
      $i++;
    }}
