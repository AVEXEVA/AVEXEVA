...
$conn = sqlsrv_connect( $serverName, $connectionInfo);
	...
	if($My_Privileges['Location']['Other_Privilege'] >= 4){
		$sQuery = "
			...
			FROM
			 (
				SELECT ROW_NUMBER() OVER ($sOrder) AS ROW_COUNT," . str_replace(" , ", " ", implode(", ", $aColumns)) . ", Contract.BAmt AS Amount
				FROM    $sTable
                        LEFT JOIN nei.dbo.Contract ON Contract.Loc = Loc.Loc
				$sWhere AND Contract.Job IS NOT NULL
			) A
