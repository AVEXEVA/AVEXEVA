...
if(isset($_SESSION['User'],$_SESSION['Hash'])){
    ...
    $My_User    = sqlsrv_query($Databases['Default'],"
        SELECT Emp.*,
...
$conn = sqlsrv_connect( $serverName, $connectionInfo);
     ...
     */
    $aColumns = array( 'Loc', 'Tag', 'Owner', 'Address', 'City', 'State', 'Zip', 'Route', 'Zone', 'Maint', 'fLong', 'Latt' );
    ...
    $sOrder = "";
    if ( isset( $_GET['order'][0]['column'] ) && $_GET['order'][0]['column'] != 12 )
    {
        $sOrder = "ORDER BY  ";
    ...
    } else if($_GET['order'][0]['column'] == 12){
        $sOrder = 'ORDER BY [Location].GPS ' . $_GET['order'][0]['dir'];
    } else if($_GET['order'][0]['column'] == 13){
        $sOrder = 'ORDER BY [Location].Distance ' . $_GET['order'][0]['dir'];
    }
	...
	if($My_Privileges['Location']['Other_Privilege'] >= 4){
		$sQuery = "
			SELECT *
			FROM
			 (
				SELECT ROW_NUMBER() OVER ($sOrder) AS ROW_COUNT," . str_replace(" , ", " ", implode(", ", $aColumns)) . ", [Location].GPS AS Geofence, [Location].Distance
				FROM $sTable
                     LEFT JOIN Portal.dbo.[Location] ON [Location].Total_Service_ID = Loc.Loc
				$sWhere
			) A
