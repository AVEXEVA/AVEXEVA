...
if(isset($_SESSION['User'],$_SESSION['Hash'])){
    ...
    $Privileged = False;
    if( isset($My_Privleges['Department']) 
	  	&& $My_Privileges['Department']['Other_Privilege'] >= 4){
            $Privileged = True;}
    if(!isset($Connection['ID'])  || !is_numeric($_GET['ID']) || !$Privileged){print json_encode(array('data'=>array()));}
    else {
        $r = sqlsrv_query($NEI,"SELECT tblWork.ID, tblWork.Super AS Department FROM nei.dbo.tblWork;");
        $Departments = array();
        if($r){
