...
if(isset($_SESSION['User'],$_SESSION['Hash'])){
    ...
    $Privileged = False;
    if( isset($My_Privileges['Invoice'], $My_Privileges['Location'])
    ...
    if(!isset($Connection['ID'])  || !is_numeric($_GET['ID']) || !$Privileged){print json_encode(array('data'=>array()));}
        ...
        $r = sqlsrv_query($NEI,"
			...
			",array($_GET['ID']));
        $data = array();
        if($r){while($array = sqlsrv_fetch_array($r,SQLSRV_FETCH_ASSOC)){$array['fDate'] = date('m/d/Y', strtotime($array['fDate']));$data[] = $array;}}
