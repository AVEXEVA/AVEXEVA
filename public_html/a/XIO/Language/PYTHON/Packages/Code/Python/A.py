...
def Lettere(tex, It):
   ...
   while len(tex) != 0 and tex != 't' and tex != '':
      if tex == '':
         continue
      t.append([Letteri(tex, 0), Letteri(tex, 1)] if len(tex) % 2 == 1 else [Letteri(tex, 0), tex[0:1], Letteri(tex, 1)])
      #text = {tex[:1] : t}
...
def Lettero(tex, It):
   for t, x in tex.items():
      if It == 1:
          return {''.join(filter(str.isalnum, str(Letteri(x, It)))) if len(x) > 0 else '' : {Letters()[t](It) : Letteri(x, It)}}
      else:
          return {Letters()[t](It) : Letteri(x, It)} if len(x) > 0 else {''.join(filter(str.isalnum, str(Letters()[t](It))))}
