...
def Lettere(tex, It):
   ...
   while len(tex) != 0 and tex != 't' and tex != '':
      t = [Letteri(tex, 0), Letteri(tex, 1)] + t
      text = {tex[:1] : t}
      tex = tex[1:]
   return text
...
def Lettero(tex, It):
   for t, x in tex.items():
      return {Letters()[t](It) : Letteri(x, It) if len(x) > 0 else ''} if not Bit(t) else {Letters()[bin(int(t))[2:3].replace('0','O').replace('1','Z')](It) : Letteri(bin(int(t))[2:3].replace('0','NE').replace('1','ERO') + bin(int(t))[3:], It)}
